...
document.getElementById('register-btn').addEventListener('click', async () => {
    ...
    if(!name || !dept || !email || !password) { alert("Sab fields lazmi hain!"); return; }
    try {
        const userCredential = await createUserWithEmailAndPassword(auth, email, password);
        await set(ref(db, `users/${userCredential.user.uid}`), { uid: userCredential.user.uid, email: userCredential.user.email, name: name, department: dept, status: 'online' });
...
async function startCall(isVideo) {
    ...
    peerConnection.ontrack = (event) => {
        if (event.streams && event.streams[0]) {
            ...
            else { remoteAudio.srcObject = event.streams[0]; remoteAudio.play(); }
        }
    };
...
function startMultiUserMeeting(meetInfo) {
    ...
    const options = { roomName: meetInfo.meetingId, width: '100%', height: '100%', parentNode: jitsiContainer, userInfo: { displayName: myFullName }, configOverwrite: { startWithAudioMuted: false, startWithVideoMuted: false, prejoinPageEnabled: false } };
...
function loadMessages() {
    ...
    onValue(ref(db, `chats/${chatId}`), (snapshot) => {
        ...
        snapshot.forEach((child) => {
            ...
            if (msg.fileData) {
                if (msg.fileType && msg.fileType.startsWith('image/')) { content += `<img src="${msg.fileData}" style="max-width:100%; border-radius:5px; margin-top:5px; cursor:pointer;" onclick="window.open('${msg.fileData}')">`; } 
                else { content += `<div style="margin-top:8px; padding:8px; background:rgba(0,0,0,0.05); border-radius:5px;">📄 <a href="${msg.fileData}" download="${msg.fileName}">Download ${msg.fileName}</a></div>`; }
            }
            msgEl.innerHTML = content; messagesDiv.appendChild(msgEl);
        });
